...
class LSTM:
    ...
    def initialize_parameters(self, x) -> None:
        ...
        self.__wx_f.set_value(np.ones(shape=shape1))
        self.__wh_f.set_value(np.ones(shape=shape2))
        # 修改遗忘门偏置初始值
        self.__b_f.set_value(np.ones(shape2[1], ))

        self.__wx_i.set_value(np.ones(shape=shape1))
        self.__wh_i.set_value(np.ones(shape=shape2))
        self.__b_i.set_value(np.ones(shape2[1], ))

        self.__wx_o.set_value(np.ones(shape=shape1))
        self.__wh_o.set_value(np.ones(shape=shape2))
        self.__b_o.set_value(np.ones(shape2[1], ))
        ...
        self.__wx_c.set_value(np.ones(shape=shape1))
        self.__wh_c.set_value(np.ones(shape=shape2))
        self.__b_c.set_value(np.ones(shape2[1], ))
    ...
    def forward(self, x, trainning):
        ...
        hstatus = np.ones((batch_size, time_step, out_units))

        # 上一步的输出
        pre_hs = self.__pre_hs
        # 第一个时间步之前，pre_hs为None，所以先进行初始化
        if pre_hs is None:
            pre_hs = np.ones((batch_size, out_units))
...
if __name__ == '__main__':
   ...
   # time_step = 2, input_dim =10
   inputs = np.ones(shape=(1, 1, 10))
   lstm = LSTM(units=20)
   lstm.initialize_parameters(inputs)
   print(lstm.forward(inputs, True))

   h0 = torch.ones(1, 1, 20)
   c0 = torch.ones(1, 1, 20)

   inputs2 = torch.ones(1, 1, 10)
   lstm2 = torch.nn.LSTM(input_size=10, hidden_size=20)
   weight_reset(lstm2)
   output, (h, c) = lstm2(inputs2, (h0, c0))
   print("=================")
   print(output)
   print(h)
   print(c)
