...
class HydroDataSetUnit11(IFedDataset):
    ...
    def check_sum(self) -> str:
        ...
        # os.path.join() 路径拼接
        if not (os.path.exists(os.path.join(self.path, 'train_unit11_12basins_t30_x.txt')) and
                os.path.exists(os.path.join(self.path, 'train_unit11_12basins_t30_y.txt')) and
                os.path.exists(os.path.join(self.path, 'test_unit11_12basins_t30_x.txt')) and
                os.path.exists(os.path.join(self.path, 'test_unit11_12basins_t30_y.txt'))):
            return ''

        sum = hashlib.md5()  # 获取一个md5加密算法对象
        with open(os.path.join(self.path, 'train_unit11_12basins_t30_x.txt'), 'rb') as file:
            sum.update(file.read())  # 制定需要加密的字符串
        with open(os.path.join(self.path, 'train_unit11_12basins_t30_y.txt'), 'rb') as file:
            sum.update(file.read())
        with open(os.path.join(self.path, 'test_unit11_12basins_t30_x.txt'), 'rb') as file:
            sum.update(file.read())
        with open(os.path.join(self.path, 'test_unit11_12basins_t30_y.txt'), 'rb') as file:
            sum.update(file.read())
        return sum.hexdigest()  # 获取加密后的16进制字符串

    def extract_files(self) -> list:
        return [
            os.path.join(self.path, 'train_unit11_12basins_t30_x.txt'),
            os.path.join(self.path, 'train_unit11_12basins_t30_y.txt'),
            os.path.join(self.path, 'test_unit11_12basins_t30_x.txt'),
            os.path.join(self.path, 'test_unit11_12basins_t30_y.txt')
        ]
    ...
    def __load_core(self, kind='train'):
        ...
        x_path = os.path.join(self.path, '%s_unit11_12basins_t30_x.txt' % kind)
        y_path = os.path.join(self.path, '%s_unit11_12basins_t30_y.txt' % kind)
...
if __name__ == '__main__':
    ...
    #     }
    # }
    hydroDataSet = HydroDataSetUnit11()
