...
from executor.federated_learning.fed_net_package import *
from executor.federated_learning.parameter_server import PSGDPSExecutor
from executor.federated_learning.fedworker import FedPSGDWorkerExecutor
...
class FedParallelSGD:
    """
        P-SGD 主调类
        P-SGD RPC Controller
    """

    def __init__(self, model: Model, data: IFedDataset, transform: ITransformer):
        """
            初始化一个P-SGD主调对象
    ...
    def parallel(self,
                 ...
                 gd_params: Sequence[Tuple[object]] = (),
        ...
        def reply(_id, x):
            if replies.get(x) is not None:
                return replies[x]
            else:
                self.__data.set_node_id(_id)
                if x == Req.Data_Package:
                    return data_package(self.__data, self.__transform)
                elif x == Req.Data_Content:
                    return data_content(self.__data, self.__transform)
        ...
        with req.request(nodes) as com:
            ...
            coordinator.submit_group(FedPSGDWorkerExecutor, assignment.nodes, self.__data.estimate_size())

            coordinator.resources_dispatch(reply)
