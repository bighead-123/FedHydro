...
class PSGDWorkerExecutor(AbsExecutor):
    ...
    def temp_test(self, model, optimizer, batch_iter, trans, data, misc):
        self.__model: [Model] = model
        self.__optimizer: [IPSGDOpContainer] = optimizer
        self.__batch_iter: [IBatchIter] = batch_iter
        self.__trans: [ITransfer] = trans
        self.__data: [IDataset] = data
        self.__misc: [misc_package] = misc
        self.__done: bool = False
    ...
    def start(self, com: ICommunication_Controller) -> None:
        ...
        for i in range(self.__misc.epoch):
            ...
            # is_stop, best_val_loss, stop_singal_num, best_weights = self.__model.is_early_stoping(i, is_stop,
            #                                                                                       best_val_loss,
            #                                                                                       stop_singal_num,
            #                                                                                       best_weights, test_x,
            #                                                                                       test_y, exp_epoch=30,
            #                                                                                       req_stop_num=req_stop_num)
            # if is_stop:
            #     print("提前退出")
            #     break
        ...
        self.__model.compile(nn.gradient_descent.SGDOptimizer(learn_rate=1e-5))  # 此处需要注意
