...
from nn.data import PSGDBlockDataFeeder, FedMetaBlockDataFeeder
from nn.model import Model
from utils.log import Logger
from dataset.extract_node_data import SingleNodeData


class PSGDMetaWorkerExecutor(AbsExecutor):
    ...
    def start(self, com: ICommunication_Controller) -> None:
        """
        1.从整合数据集中得到每个节点对应的数据support_x, support_y, query_x, query_y
        2.每次从support_x, support_y中随机抽取K-shot个数据样本meta_support_x，meta_support_y，
          从query_x, query_y随机抽取K_shot个数据样本meta_query_x, meta_query_y
        3.使用meta_support_x, meta_support_y更更新模型参数 w--->w'
        4.使用meta_query_x, meta_query_y计算损失对应的梯度grad
        5.上传至参数服务器
        6.从参数服务器得到更新后的模型参数
        7.重复执行1~6
        :param com:
        ...
        support_x, support_y = train_single_node_data.extract_data()
        # block_ids[-1]当前节点block_id
        test_single_node_data = SingleNodeData(test_x, test_y, block_ids[-1], self.__batch_iter)
        query_x, query_y = test_single_node_data.extract_data()

        self.__log.log_message('Dataset is ready, type: ({})'.format(self.__data))
        # build 01 feeder

        # 除了block_ids其他都用不到
        feeder = PSGDBlockDataFeeder(support_x, support_y, batch_iter=self.__batch_iter, block_ids=block_ids)
        # assemble optimizer
        ...
        # global initialization, for better improvement
        self.__model.global_initialization()
        # do until reach the target accuracy
        for i in range(self.__misc.epoch):
            # change title
            self.__log.Title = log_head + "-Epo-{}".format(i + 1)
            # 执行一步更新参数
            random_points = np.random.choice(np.arange(len(support_x)), k_shot, replace=False)[:, None]
            meta_support_x = support_x[random_points]
            meta_support_y = support_y[random_points]
            self.__model.fit_meta(meta_support_x, meta_support_y, epoch=i, printer=self.__log)

            # 在查询集上测试，得到梯度
            random_points = np.random.choice(np.arange(len(query_x)), k_shot, replace=False)[:, None]
            meta_query_x = query_x[random_points]
            meta_query_y = query_y[random_points]
            self.__model.query_evaluate(meta_query_x, meta_query_y)  # do tests on query set
