...
class Coordinator:
    ...
    # 2、控制了事务的确认，通过状态标志集合（𝑛𝑜𝑑𝑒_𝑟𝑒𝑎𝑑𝑦）来记录节点的状态
    ...
    # 控制了事务的最终执行和事务执行结果的回收，同样是通过标志集合（𝑛𝑜𝑑𝑒_𝑟𝑒𝑎𝑑𝑦）实现
    def join(self) -> Dict[int, object]:
        ...
        while node_ready != self.allocated_nodes:
            ...
            if isinstance(data, IReplyPackage):
                data.restore()
                self.__log.log_message('Restoring 01 ({}) from {}.'.format(data, id_from))
